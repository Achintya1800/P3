...
# UPDATED Generator class to match your improved training model
class Generator(nn.Module):
    def __init__(self, latent_dim, img_shape):
        super(Generator, self).__init__()
        self.img_shape = img_shape
        ...
        # UPDATED architecture to match improved training model
        self.model = nn.Sequential(
            # First layer - expanded capacity
            nn.Linear(latent_dim + 10, 512),  # Changed from 256 to 512
            nn.BatchNorm1d(512),  # Added BatchNorm
            nn.LeakyReLU(0.2, inplace=True),  # Changed to LeakyReLU
            nn.Dropout(0.3),  # Added dropout
            
            # Second layer - much larger
            nn.Linear(512, 1024),  # Changed from 512 to 1024
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.3),
            
            # Third layer - additional layer
            nn.Linear(1024, 1024),  # New layer
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.2),
            
            # Output layer
            nn.Linear(1024, 784),  # Changed from 512 to 1024 input
            nn.Tanh()
        )
...
with col_info1:
    ...
    **Dataset:** MNIST handwritten digits  
    **Training:** ~40 epochs with CNN-quality architecture  
    """)
